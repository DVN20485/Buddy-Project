...
void *balloc(size_t size) {
    ...
    int lvl = level(size);  //get level of size
    head *block = findSmallestFree(lvl);

    if(block == NULL) //no space we add new block
    {
       block = new();
       addToLinkedListFront(block);
    }

    //split block
    block = splitNodesForLevel(lvl, block);
    block ->status = STATUS_USED;
    return hide(block);

    /*if(flists[lvl] != NULL)
    {
        if(block->level == lvl){ //free space on teh level requested
                block->status = STATUS_USED;
                return hide(block);
            }
            else{               //current block is full so make a new one on the same level
                block = splitNodesForLevel(lvl, block);
                block->status = STATUS_USED;
                return hide(block);
            }
